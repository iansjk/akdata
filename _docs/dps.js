...
<div class="card">
  ...
  for (let i = 0; i < charColumnCount; i++) {
    ...
    $dps
      .find(".dps__row-level")
      .append(
        `<td><div class="container"><div class="form-group row mb-0"><select class="form-control form-control-sm col-7 dps__phase" data-index="${i}"></select><select class="form-control form-control-sm col-5 dps__level" data-index="${i}"></select></div></div></td>`
      );
    $dps
      .find(".dps__row-atk")
      .append(`<td><div class="dps__atk" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-attackSpeed")
      .append(
        `<td><div class="dps__attackSpeed" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-baseAttackTime")
      .append(
        `<td><div class="dps__baseAttackTime" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-dps")
      .append(`<td><div class="dps__dps" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-skill")
      .append(
        `<td><div class="container"><div class="form-group row mb-0"><select class="form-control form-control-sm col-7 dps__skill" data-index="${i}"></select><select class="form-control form-control-sm col-5 dps__skilllevel" data-index="${i}"></select></div></div></td>`
      );
    $dps
      .find(".dps__row-s_atk")
      .append(`<td><div class="dps__s_atk" data-index="${i}"></div></td>`);

    $dps
      .find(".dps__row-s_dps")
      .append(
        `<td><div class="dps__s_dps font-weight-bold" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-a_dps")
      .append(
        `<td><div class="dps__a_dps font-weight-bold" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-n_dps")
      .append(`<td><div class="dps__n_dps" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-g_dps")
      .append(
        `<td><div class="dps__g_dps font-weight-bold" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-s_att")
      .append(
        `<td><div class="dps__s_att font-weight-bold" data-index="${i}"></div></td>`
      );
    $dps
      .find(".dps__row-n_att")
      .append(`<td><div class="dps__n_att" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-s_diff")
      .append(`<td><div class="dps__s_diff " data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-g_diff")
      .append(`<td><div class="dps__g_diff " data-index="${i}"></div></td>`);
    // $dps.find('.dps__row-e_time').append(`<td><div class="dps__e_time" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-s_damage")
      .append(`<td><div class="dps__s_damage" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-period")
      .append(`<td><div class="dps__period" data-index="${i}"></div></td>`);
    $dps
      .find(".dps__row-results")
      .append(
        `<td><a class="dps__results" data-index="${i}" href="#">[点击显示]</a></td>`
      );
    $dps
      .find(".dps__row-note")
      .append(`<td><div class="dps__note" data-index="${i}"></div></td>`);
    $dps.find(".dps__row-prts").append(`<td></td>`);
    $dps.find(".dps__row-option").append(`<td></td>`);
    $dps
      .find(".dps__row-damagepool")
      .append(
        `<td><a class="dps__damagepool" data-index="${i}" href="#">[点击显示]</a></td>`
      );
    $dps
      .find(".dps__row-anim")
      .append(
        `<td><a class="dps__anim" data-index="${i}" href="#">[点击显示]</a></td>`
      );
    $dps
      .find(".dps__row-potentialrank")
      .append(
        `<td><select class="form-control form-control-sm dps__potentialrank" data-index="${i}">${[
          0, 1, 2, 3, 4, 5,
        ]
          .map((x) => `<option value="${x}">${x + 1}</option>`)
          .join("")}</select></td>`
      );
    $dps.find(".dps__row-favor").append(
      `<td><select class="form-control form-control-sm dps__favor" data-index="${i}">${Object.keys(
        new Array(51).fill(0)
      )
        .map((x) => `<option value="${x * 2}">${x * 2}</option>`)
        .join("")}</select></td>`
    );
    $dps
      .find(".dps__row-equip")
      .append(
        `<td><select class="form-control form-control-sm dps__equip" data-index="${i}"></select></td>`
      );
  }
  ...
  $(".dps__skill, .dps__skilllevel, .dps__potentialrank, .dps__favor").change(
    chooseSkill
  );
  $(
    ".dps__enemy-def, .dps__enemy-mr, .dps__enemy-count, .dps__enemy-hp"
  ).change(calculateAll);
  $(
    ".dps__buff-atk, .dps__buff-atkpct, .dps__buff-ats, .dps__buff-cdr, .dps__buff-batk, .dps__buff-scale"
  ).change(calculateAll);
...
function showDetail() {
  ...
  pmBase.component.create({
    ...
    content: markdown
      .makeHtml(Characters[index].dps.log)
...
`;
  ...
  for (let row = 0; row < 2; ++row) {
    ...
    let pool = [0, 1, 2, 3].map(
      (x) => Math.round(d.damagePool[x] + d.extraDamagePool[x]),
      0
    );
    ...
    let data = [
      Math.round(d.atk),
      `${d.attackTime.toFixed(3)}s<br>${Math.round(d.attackTime * 30)}帧`,
      Math.round(d.dur.hitCount),
      d.dur.duration.toFixed(2),
      ...pool,
    ];
...
function updateChar(charId, index) {
  ...
  let html = [...Array(phaseCount).keys()]
    .map((x) => `<option value="${x}">精英${x}</option>`)
    .join("");
  ...
  $(`.dps__row-prts td:nth-child(${index + 2})`).html(
    `<a href="http://prts.wiki/w/${charData.name}#.E6.8A.80.E8.83.BD" target="_blank">点击打开</a>`
  );
  ...
  if (opts.char[charId]) {
    for (var t of opts.char[charId]) {
      ...
      if (t == "cond" && opts.cond_info[charId]) {
        ...
        let talent = null;
        if (typeof which == "number") talent = charData.talents[which - 1];
        else if (which == "trait") talent = charData.trait;
        else if (which.text) {
          text = "触发 - " + which.text;
...
function calculate(index) {
  ...
  getElement("s_atk", index)
    .html(`<b>${Math.round(s.atk)}</b>`)
    .css("color", DamageColors[s.damageType]);
  ...
  if (s.damageType != 2) {
    ...
    if (s.critDamage > 0)
      line += ` + ${s.critDamage.toFixed(2)} * ${s.dur.critHitCount}`;
    if (s.extraDamage > 0) line += ` + ${s.extraDamage.toFixed(2)}`;
    var tmp =
      s.hitDamage * s.dur.hitCount +
      (s.critDamage * s.dur.critHitCount || 0) +
      s.extraDamage;
  ...
  if (s.hps == 0 || s.dps == 0) {
    getElement("s_dps", index)
      .html(Math.round(s.dps || s.hps))
      .css("color", color);
  } else {
    getElement("s_dps", index)
      .html(`DPS: ${Math.round(s.dps)}, HPS: ${Math.round(s.hps)}`)
      .css("color", color);
  }
  // attack dps
  if (s.attackTime > 0) {
    getElement("a_dps", index)
      .html(Math.round(s.hitDamage / s.attackTime))
      .css("color", color);
  } else {
  ...
  if (dps.normal.hps == 0 || dps.normal.dps == 0) {
    color =
      dps.normal.dps == 0
        ? DamageColors[2]
        : DamageColors[dps.normal.damageType];
    getElement("n_dps", index)
      .html(Math.round(dps.normal.dps || dps.normal.hps))
      .css("color", color);
  } else {
    getElement("n_dps", index)
      .html(
        `DPS: ${Math.round(dps.normal.dps)}, HPS: ${Math.round(dps.normal.hps)}`
      )
      .css("color", color);
  }
  // period
  if (dps.normal.dur.stunDuration > 0)
    getElement("period", index).html(
      `眩晕${dps.normal.dur.stunDuration}s + ${
        Math.round(dps.normal.dur.duration * 100) / 100
      }s + ${Math.round(s.dur.duration * 100) / 100}s`
    );
  else if (dps.skill.dur.prepDuration > 0)
    getElement("period", index).html(
      `${Math.round(dps.normal.dur.duration * 100) / 100}s + 准备${
        dps.skill.dur.prepDuration
      }s + ${Math.round(s.dur.duration * 100) / 100}s`
    );
  else
    getElement("period", index).html(
      `${Math.round(dps.normal.dur.duration * 100) / 100}s + ${
        Math.round(s.dur.duration * 100) / 100
      }s`
    );
  ...
  if (s.dur.tags.includes("auto")) {
    ...
    else getElement("period", index).append(" / 落地点火");
  }
  ...
  if (dps.globalDps == 0 || dps.globalHps == 0)
    getElement("g_dps", index)
      .html(Math.round(dps.globalDps || dps.globalHps))
      .css("color", DamageColors[dps.normal.damageType]);
  else
    getElement("g_dps", index).html(
      `DPS: ${dps.globalDps.toFixed(1)}, HPS: ${dps.globalHps.toFixed(1)}`
    );
  //  getElement('e_time', index).html(dps.killTime ?  `${Math.ceil(dps.killTime)}秒` : '-');
  getElement("note", index).html(
    dps.note.replace(/\n/g, "<br>").replace(/ /g, "&nbsp;")
  );

  // attack time
  getElement("s_att", index).text(
    `${dps.skill.attackTime.toFixed(3)} s / ${dps.skill.frame} 帧`
  );
  getElement("n_att", index).text(
    `${dps.normal.attackTime.toFixed(3)} s / ${dps.normal.frame} 帧`
  );
  console.log(dps);
}
