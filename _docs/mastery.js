...
function checkSpecs(tag, spec) {
  ...
  if (tag in specs && spec in specs[tag]) return specs[tag][spec];
  else return false;
}
...
function buildVueModel() {
  ...
  for (let id in AKDATA.Data.character_table) {
    ...
    if (!data.phases || data.phases.length <= 2) excludeList.push(id);
  }
    ...
    methods: {
      ...
      selChar: function (event) {
        AKDATA.selectCharCallback = function (id) {
          window.vue_app.charId = id;
          window.vue_app.changeChar();
        };
...
*/
        for (var sk in rv.cost) {
          matsView[sk] = {
            title: rv.skill[sk] || `精英化/模组材料(不含龙门币)`,
          };
          matsView[sk].list = [];
          tot = 0;
          ...
          for (i = 0; i < rv.mats[sk].length; ++i) {
            ...
            let lineTitle = `${
              base_lv + i
            } <i class="fas fa-angle-right"></i> ${base_lv + i + 1}`;
            if (sk.includes("elite") && i == 2)
              // 专武材料，非等级
      ...
      updateLevelingTable: function () {
        ...
        var r = {};
        $.extend(r, DefaultAttribute);
        $.extend(r, Stages["基准"]);
        var ch0 = buildChar(this.charId, db.skills[0].skillId, r);
        r.level = "max";
        var ch1 = buildChar(this.charId, db.skills[0].skillId, r);
        var a0 = AKDATA.attributes.getCharAttributes(ch0),
          a1 = AKDATA.attributes.getCharAttributes(ch1);

        var gain_pct = [
          (a1.maxHp - a0.maxHp) / a0.maxHp,
          (a1.atk - a0.atk) / a0.atk,
          (a1.def - a0.def) / a0.def,
        ];
        var gain = [a1.maxHp - a0.maxHp, a1.atk - a0.atk, a1.def - a0.def];
        var result = [0, 1, 2].map(
          (x) => `+${Math.round(gain[x])} (+${(gain_pct[x] * 100).toFixed(1)}%)`
        );
        ...
        // 模板分析
        var scalar = [
          ...gain_pct.map((x) => x * 100),
          a0.baseAttackTime * 1000,
        ];
...
function buildChar(charId, skillId, recipe) {
  ...
  if (recipe.level == "max" || recipe.level > maxLevel) char.level = maxLevel;
  else char.level = recipe.level;
  char.name = db.name;
  char.skillName = skilldb.levels[char.skillLevel].name;
  //console.log(char);
  var _opts = AKDATA.Data.dps_options.char[charId];
  if (
    checkSpecs(charId, "use_token_for_mastery") ||
    checkSpecs(skillId, "use_token_for_mastery")
  )
  ...
  if (recipe.equip) {
    ...
    let equips = Object.keys(edb).filter(
      (x) => edb[x].charId == charId && edb[x].unlockLevel > 0
    );
    // console.log(equips);
    if (equips.length > 0) {
...
function calculate(charId) {
  ...
  if (
    ["char_306_leizi", "char_472_pasngr", "char_4004_pudd"].includes(charId)
  ) {
    enemy.count = 2;
  }
  ...
  db.skills.forEach((skill) => {
    var entry = {};
    recipe = {};
    $.extend(recipe, DefaultAttribute);
    console.log(recipe);
  ...
  Object.keys(resultView.mats).forEach((k) => {
    resultView.cost[k] = resultView.mats[k].map((item) =>
      [0, ...Object.keys(item)].reduce(
        (sum, x) => sum + greenTable[x] * item[x]
      )
    );
  });
...
function buildChartView(resultView, key) {
  ...
  for (let stg in view.stages) {
    ...
    for (let skill in view.skill) {
      ...
      if (skill in last) entry.push((value - last[skill]).toFixed(2));
      else entry.push(value.toFixed(2));
      last[skill] = value;
    }
    columns.push(entry);
    groups.push(stg);
  ...
  for (let skill in view.skill) {
    ...
    if (view.dps[skill]["满潜"].spType != 8) {
      ...
      if (view.dps[skill]["满潜"].s_ssp <= 0) line += " （落地点火）";
    }
...
function updatePlot(chartView) {
  window.chart["chart"].load({
    columns: chartView.columns,
    groups: chartView.groups,
  });
  setTimeout(function () {
    $(".c3-chart-bar.c3-target-基准").css("opacity", 0.4);
...
function buildPieView(chartView) {
  ...
  for (var i = 0; i < chartView.skill_names.length; i += 1) {
    ...
    view[sk] = [];
    for (var j = 1; j < chartView.groups.length; j += 1) {
      // skip base value
...
function calcSubClass(x, prof) {
  ...
  var dists = Object.keys(sub_db)
    .filter((k) => sub_db[k].prof == prof)
    .map((k) => [k, _dist(x, sub_db[k].feat)])
    .sort((x, y) => x[1] - y[1]);
...
function plot2(chartView) {
  ...
  */
  ...
  var series = [];
  for (var i = 0; i < chartView.groups.length; ++i) series.push(seriesTemplate);
