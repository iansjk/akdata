...
function buildVueModel() {
  ...
  for (let id in AKDATA.Data.character_table) {
    ...
    if (!data.skills || data.skills.length == 0) excludeList.push(id);
  }
  ...
  window.vue_app = new Vue({
    ...
    methods: {
      ...
      explainChar: function (char) {
        ...
        var skillStr = skillDB[char.skillId]
          ...
          : "";
        var equipStr =
          char.equipId.length > 0
            ? equipDB[char.equipId].uniEquipName + "<br>"
            : "";
        var optionStr = char.options
          .map((x) => optionDB.tags[x].displaytext)
          .join("/");
        return {
          name: charDB[char.charId].name,
          text: levelStr + equipStr + skillStr,
          option: optionStr,
        };
      },
      explainArgs: function () {
        ...
        if (this.enemy.magicResistance)
          line += `/ 法抗 ${this.enemy.magicResistance}`;
      ...
      setChar: function (event) {
        ...
        var opts = (optionDB.char[this.charId] || []).map((x) => ({
          tag: x,
          text: optionDB.tags[x].displaytext,
        }));
        ...
        var equips = Object.keys(equipDB).filter(
          (x) => equipDB[x].charId == this.charId
        );
        this.opt_equip = equips.map((x) => ({
          id: x,
          name: equipDB[x].uniEquipName,
        }));
        this.details.equipId =
          equips.length > 0 ? equips[equips.length - 1] : "";
      },
    ...
    computed: {
        ...
        get: function () {
          ...
          this.plotList.forEach((x) => {
            ...
            if (!this.resultCache[hash]) {
              this.resultCache[hash] = AKDATA.attributes.calculateDpsSeries(
                args.char,
                args.enemy,
                args.raidBuff,
                args.enemyKey,
                EnemySeries[args.enemyKey]
              );
            }
      ...
      columns: function () {
        ...
        for (var i = 0; i < this.plotList.length; ++i) {
          ...
          var values = Object.keys(this.chartView[i]).map(
            (k) => this.chartView[i][k][this.chartKey]
          );
    ...
    watch: {
      ...
      columns: function () {
        ...
        window.chart.axis.labels({
          x: this.explainArgs(),
          y: LabelNames[this.chartKey],
        });
      },
      chartKey: function () {
        this.buildChartView();
